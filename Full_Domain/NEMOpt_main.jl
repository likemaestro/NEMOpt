...
ELAPSED_TIME = @elapsed begin
    ...
    p, ϵ = 1, 5e-4                                          # Penalty factor, Minimum density.
    ...
    N_ITER = 120                                            # Specify the maximum number of iterations for the optimization.
    V_MAX = 0.5                                             # Specify the maximum volume fraction for the topology.
    ...
    Description = ["Optimizing for Negative Poisson's ratio."]         # Describe the current analysis.
    ...
    N_Elements, Ns, NodeNumbers, NC, XY, elCoords, DOFs, DOFs_Filter, D1, D2, D1_Filter = Generate_Mesh(Lx, Ly, Nelx, Nely)
    ...
    function Volume_Constraint_Grad(𝜌_Design)
        ...
        begin
            ...
            Fm = Vf_F[1:1*Ns[1], :]
            Fs = Vf_F[1*Ns[1]+1:1*(Ns[1]+Ns[2]), :]
            Fi = Vf_F[1*(Ns[1]+Ns[2])+1:1*(Ns[1]+Ns[2]+Ns[3]), :]
        ...
        Vf_δθ_Unfiltered = zeros(Int(N_Elements / 4), 1)
        for k = 1:Int(N_Elements / 4)
            el = elements_BL[k]
            i, j = Int(ceil(el / Nely)), (el - 1) % Nely + 1
            elV, elH, elVH = i * Nelx - j + 1, (Nely - i) * Nelx + j, (Nely - i) * Nelx + Nelx - j + 1
            Vf_δθ_Unfiltered[k] = Vf_δθ_Full_Unfiltered[el] + Vf_δθ_Full_Unfiltered[elV] + Vf_δθ_Full_Unfiltered[elH] + Vf_δθ_Full_Unfiltered[elVH]
        end
    ...
    addvar!(model, zeros(Int(N_Elements / 4)), ones(Int(N_Elements / 4)))
